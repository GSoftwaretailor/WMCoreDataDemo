...
@property(nonatomic,strong) MStudent* entity;
@end

@implementation EditStudent

-(instancetype)initWithItem:(MStudent *)item{
    self = [super init];
    if(self){
        _entity = item;
    }
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    self.txtSex.text =[NSString stringWithFormat: @"%@", self.entity.sex];
    self.txtAge.text =[NSString stringWithFormat: @"%@", self.entity.age];
}
...
//    MStudent* item = [NSEntityDescription insertNewObjectForEntityForName: @"Student" inManagedObjectContext:[CoreDataManager shareManager].context];
    [self.entity setUserName:self.txtUserName.text];
    [self.entity setSex:@([self.txtSex.text integerValue])];
    [self.entity setSex:@([self.txtAge.text integerValue])];
